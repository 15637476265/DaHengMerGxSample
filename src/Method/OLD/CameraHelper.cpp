...
void CameraHelper::connectDevice()
{
...
GX_STATUS CameraHelper::OpenDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS CameraHelper::CloseDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS CameraHelper::InitDevice()
{
    ...
    int exposure = 800;//_sys_config.get<int>("CameraHelper.Exposure",0);
...
bool CameraHelper::PrepareForShowImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    bool ret = false;
...
bool  CameraHelper::PrepareForShowColorImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    //创建QImage对象，用于显示图像
...
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    return const_cast<CameraHelper*>(this)->takePicture() ;
}

GX_FRAME_DATA CameraHelper::getFrameDate()
{
    return m_frame_data;
}

cv::Mat CameraHelper::takePicture()
{
...
void CameraHelper::ShowErrorString(GX_STATUS error_status)
{
